...
> **[2026-07-15 업데이트]** HTTPie CLI는 개발이 정체되어 버전이 3.2.x에 머물러 있다. 여전히 사용하는 데는 문제가 없다. 또한 아래 설치 명령어 중 `sudo echo ... >` 형태는 리다이렉션에 sudo가 적용되지 않아 동작하지 않는 문제가 있어 `sudo tee` 방식으로 수정했다.
