...
> **[2026-07-15 업데이트]** 아래 settings.json 예제에 있는 `terminal.integrated.shell.windows` 설정은 지원 중단(deprecated)되었다. 현재는 `terminal.integrated.defaultProfile.windows`와 `terminal.integrated.profiles.windows`를 사용한다. 폰트 설정 방법은 지금도 그대로 유효하다.

VSCode는 [chocolatey](https://chocolatey.org)로 깔았고 설치 정보는 아래와 같다. (작성 당시 버전 기준이며, 지금 보면 꽤 오래된 버전이다.)
...
> 위 예제의 `terminal.integrated.shell.windows`는 더 이상 사용되지 않는 설정이다. 현재는 아래와 같이 프로파일 방식으로 설정한다.
