...
categories: tools
redirect_from:
  - /tools/2020/09/11/jekyll_wsl2.html
---
...
> **[2026-07-15 업데이트]** 이 글에서 사용한 Ubuntu 20.04 LTS와 Ruby 2.7은 모두 지원이 종료(EOL)되었다. 현재는 WSL에 Ubuntu 24.04 LTS를 설치하고 아래 "2026년 기준 설치 방법" 절의 절차를 따르는 것을 권장한다. 본문의 나머지 내용은 당시 기록으로 남겨둔다.

## 2026년 기준 설치 방법 (Ubuntu 24.04 LTS)

[Jekyll 공식 Ubuntu 설치 가이드](https://jekyllrb.com/docs/installation/ubuntu/) 기준의 최신 절차는 다음과 같다.

```bash
$ sudo apt install ruby-full build-essential zlib1g-dev
```

root로 gem을 설치하지 않도록 사용자 홈 디렉토리에 gem 설치 경로를 지정한다.
...
이후 저장소 디렉토리에서 `bundle install` 후 `bundle exec jekyll serve`로 구동하면 된다.
...
> **[2026-07-15 업데이트]** 아래처럼 Gemfile.lock을 직접 수정하는 방식은 더 이상 필요하지 않고 권장되지도 않는다. Gemfile에 버전 제약을 지정한 뒤 `bundle update kramdown` 명령으로 Bundler가 Gemfile.lock을 갱신하도록 하는 것이 올바른 방법이다.
