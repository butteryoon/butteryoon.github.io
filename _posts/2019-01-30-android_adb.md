...
> **[2026-07-15 업데이트]** 원래 소개했던 "Minimal ADB and Fastboot"의 다운로드처(androidfilehost.com)가 서비스를 종료해 더 이상 받을 수 없다. 지금은 구글이 공식 배포하는 [SDK Platform Tools](https://developer.android.com/tools/releases/platform-tools) zip을 받거나 `winget install Google.PlatformTools` 명령으로 설치하면 된다. 설치 방법(1번 항목)을 이에 맞게 다시 정리했다. adb 사용법(2번 이후)은 그대로 유효하다.
...
구글 공식 [SDK Platform Tools](https://developer.android.com/tools/releases/platform-tools) 페이지에서 Windows용 zip을 다운로드 후 특정폴더에 압축을 풀면 아래와 같이 adb와 dll이 보인다. 

winget을 사용할 수 있는 환경이면 명령어 한 줄로 설치할 수도 있다. 

```powershell
winget install Google.PlatformTools
```

> 설치된 디렉토리로 이동하여 쓸 수도 있지만  "제어판\시스템 및 보안\시스템 > 고급 시스템 설정 > 환경변수 에 압축을 푼 platform-tools 경로를 Path 사용자변수에 추가하면 편리하다.
> Windows 10/11의 경우 왼쪽 하단 검색툴에서 "시스템 환경 변수 편집"으로 검색한다. 

```powershell
$ ls
adb.exe*  AdbWinApi.dll*  AdbWinUsbApi.dll*  fastboot.exe*  etc1tool.exe*  make_f2fs.exe* ...
```
...
- [SDK Platform Tools (공식)](https://developer.android.com/tools/releases/platform-tools)
