...
실시간 영상 스트리밍 서버를 테스트하기 위해 인코딩된 MP4 파일을 rtsp 전송 및 수신을 ffmpeg을 이용해서 시험해본다. 

## ffplay LISTEN 모드로 실행. 

ffplay를 LISTEN 모드로 실행하고 영상이 인입되면 1024x720 해상도로 변경하여 재생한다. 
...
## ffmpeg RTSP Restream

ffmpeg로 인코딩된 mp4 파일을 열어 8888 포트로 전송한다. 
