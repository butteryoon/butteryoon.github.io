...
네이버 2단계 인증을 켠 상태에서 아이폰 기본 Mail 앱에 네이버 메일을 연동하는 방법을 정리한다.

<!--more-->

> **[2026-07-15 업데이트]** 작성 당시와 메뉴 이름/경로가 달라졌다. 네이버는 [내정보(보안설정)](https://nid.naver.com/user2/help/myInfo)에서 **보안설정 > 2단계 인증 > 애플리케이션 비밀번호 관리** 순으로 들어가면 되고, 아이폰은 iOS 14 이후 "설정 > 암호 및 계정"이 **설정 > Mail > 계정 > 계정 추가**로 바뀌었다. 전체 흐름(IMAP 활성화 → 애플리케이션 비밀번호 생성 → 계정 추가)은 지금도 동일하므로 아래 원문을 참고하면 된다.
