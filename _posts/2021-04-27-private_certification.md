...
> **[2026-07-15 업데이트]** openssl 명령어는 지금도 그대로 유효하다. 다만 아래 파이썬 테스트 서버 예제에서 사용한 `ssl.wrap_socket()` 함수는 Python 3.12에서 **완전히 제거**되어 최신 파이썬에서는 동작하지 않는다. `ssl.SSLContext`를 사용하는 예제를 추가해 두었으니 그쪽을 사용한다. 인터넷 익스플로러(IE)도 2022년 6월 지원이 종료되어 관련 안내는 참고용으로만 남겨둔다.
...
> *(2026-07 주석)* 위에서 사용한 `ssl.wrap_socket()`은 Python 3.7부터 deprecated 되었고 **Python 3.12에서 제거**되었다. 최신 파이썬에서는 아래와 같이 `SSLContext`를 사용해야 한다.
...
context.load_cert_chain(certfile='/home/opc/.ssl/cert.pem', keyfile='/home/opc/.ssl/key.pem')
...
> *(2026-07 주석)* 인터넷 익스플로러는 2022년 6월에 지원이 종료되었다. 다만 Windows에서 인증서를 "신뢰할 수 있는 루트 인증 기관" 저장소에 등록하면 Edge, Chrome 등 다른 브라우저에서도 동일하게 경고 없이 테스트할 수 있다.
